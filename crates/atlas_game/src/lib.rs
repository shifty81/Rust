//! `atlas_game` — gameplay systems and plugins.
//!
//! This crate contains all actual game logic.  It is compiled into both the
//! shipped runtime (`atlas_runtime_app`) and the editor's PIE runtime world
//! (`atlas_editor_play`).  It must not depend on any `atlas_editor_*` crate.
...
/// sub-app inside `atlas_editor_play`.
