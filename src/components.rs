...
pub struct OrbitalBody {
    /// Distance from `orbit_center` (metres).
    pub orbit_radius: f32,
    /// Time for one full orbit (seconds).
    pub orbit_period: f32,
    /// Current angle in the orbit (radians).
    pub orbit_angle: f32,
    /// World-space position of the focus point.
    pub orbit_center: Vec3,
    /// Axis of rotation for the orbit (default Y).
    pub orbit_axis: Vec3,
}
...
impl Default for WorldTime {
    fn default() -> Self {
        Self {
            day_fraction: 0.25, // start just after dawn
            total_days: 0.0,
        }
    }
}
