...
fn generate_pending_chunks(
    ...
    while generated < MAX_CHUNKS_PER_FRAME {
        ...
        let voxels = generate_chunk_data(coord, &height_fbm, &moisture_fbm);
        let Some(mesh) = build_chunk_mesh(&voxels) else {
    ...
    for lx in 0..cs {
        for ly in 0..cs {
            for lz in 0..cs {
                ...
                let terrain_radius = PLANET_RADIUS + h_raw * MAX_TERRAIN_HEIGHT;

                if dist > terrain_radius + 1.0 {
                    if dist <= SEA_LEVEL {
                        let idx = voxel_index(lx as usize, ly as usize, lz as usize);
                ...
                let depth = ((terrain_radius - dist).max(0.0) as u32).min(255);
                let altitude = terrain_radius - PLANET_RADIUS;
...
fn build_chunk_mesh(voxels: &[Voxel]) -> Option<Mesh> {
    let cs = CHUNK_SIZE as i32;
...
fn face_vertices(ox: f32, oy: f32, oz: f32, dx: i32, dy: i32, dz: i32) -> [[f32; 3]; 4] {
    match (dx, dy, dz) {
        (1, 0, 0)  => [[ox+1.0, oy,     oz    ], [ox+1.0, oy+1.0, oz    ], [ox+1.0, oy+1.0, oz+1.0], [ox+1.0, oy,     oz+1.0]],
        (-1, 0, 0) => [[ox,     oy,     oz+1.0], [ox,     oy+1.0, oz+1.0], [ox,     oy+1.0, oz    ], [ox,     oy,     oz    ]],
        (0, 1, 0)  => [[ox,     oy+1.0, oz    ], [ox,     oy+1.0, oz+1.0], [ox+1.0, oy+1.0, oz+1.0], [ox+1.0, oy+1.0, oz    ]],
        (0, -1, 0) => [[ox,     oy,     oz+1.0], [ox,     oy,     oz    ], [ox+1.0, oy,     oz    ], [ox+1.0, oy,     oz+1.0]],
        (0, 0, 1)  => [[ox+1.0, oy,     oz+1.0], [ox+1.0, oy+1.0, oz+1.0], [ox,     oy+1.0, oz+1.0], [ox,     oy,     oz+1.0]],
        _          => [[ox,     oy,     oz    ], [ox,     oy+1.0, oz    ], [ox+1.0, oy+1.0, oz    ], [ox+1.0, oy,     oz    ]],
    }
}
